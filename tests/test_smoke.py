"""Import-smoke + pure-function tests for provenance-atlas (Bucket-B no-tests remediation).
...
_SCRIPT = os.path.join(
    ...
    'scripts', 'build_provenance_atlas.py')


def _load():
    sys.path.insert(0, os.path.dirname(_SCRIPT))
    spec = importlib.util.spec_from_file_location("buildmod_provenance_atlas", _SCRIPT)
...
def test_slugify_contract():
    ...
    assert mod.slugify("--Edge--Case--") == "edge-case"
    assert mod.slugify("A_B.C") == "a-b-c"
